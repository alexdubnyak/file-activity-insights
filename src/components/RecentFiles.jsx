import React, { useState } from 'react';
import Toggle from './Toggle';
import './RecentFiles.css';

// Иконка recent-files из assets/side-bar
const RecentFilesIcon = () => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path 
      d="M10 4.2C13.2 4.2 15.8 6.8 15.8 10C15.8 13.2 13.2 15.8 10 15.8C6.8 15.8 4.2 13.2 4.2 10C4.2 6.8 6.8 4.2 10 4.2ZM10 3C6.1 3 3 6.1 3 10C3 13.9 6.1 17 10 17C13.9 17 17 13.9 17 10C17 6.1 13.9 3 10 3Z" 
      fill="currentColor"
    />
    <path 
      d="M12 11V10H10V6H9V11H12Z" 
      fill="currentColor"
    />
  </svg>
);

const RecentFiles = ({
  enabled = true,
  onToggle = null,
  className = '',
  ...props
}) => {
  const [isEnabled, setIsEnabled] = useState(enabled);

  const handleToggleChange = (newState) => {
    setIsEnabled(newState);
    if (onToggle) {
      onToggle(newState);
    }
  };

  return (
    <div 
      className={`recent-files ${className}`}
      {...props}
    >
      <div className="recent-files__content">
        <div className="recent-files__icon">
          <RecentFilesIcon />
        </div>
        <span className="recent-files__label">
          Recent files
        </span>
      </div>
      
      <div className="recent-files__toggle">
        <Toggle
          initialState={isEnabled}
          onToggle={handleToggleChange}
          size="small"
        />
      </div>
    </div>
  );
};

export default RecentFiles;
